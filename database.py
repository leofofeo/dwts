...
def add_dwts_team(contestant_name: str, partner_name: str, division: str):
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?)
    """, (contestant_name, partner_name, division))
...
def eliminate_team(dwts_team_id: int, week_number: int):
    ...
    cursor.execute("""
        UPDATE dwts_teams
        SET is_eliminated = 1, elimination_week = ?
        WHERE id = ?
    """, (week_number, dwts_team_id))
    conn.commit()
    conn.close()
...
def is_player_eliminated(player_id: int) -> bool:
    ...
    cursor.execute("""
        ...
        FROM dwts_teams dt
        JOIN player_picks pp ON dt.id = pp.dwts_team_id
        WHERE pp.player_id = ? AND dt.is_eliminated = 0
    """, (player_id,))
    result = cursor.fetchone()
    conn.close()
...
def get_weekly_scores_for_team(dwts_team_id: int, week_number: int):
    ...
    cursor.execute("""
        ...
        FROM weekly_scores
        WHERE dwts_team_id = ? AND week_number = ?
        ORDER BY judge_name
    """, (dwts_team_id, week_number))
...
def get_latest_week() -> int:
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT COALESCE(MAX(week_number), 0) as max_week FROM weekly_scores")
    result = cursor.fetchone()
    conn.close()
